...
- **Мінімальні залежності хостів**: main — лише Docker; agent — лише Docker
  (+ `amd-smi` на хості для GPU-телеметрії, якщо є GPU; контейнер отримує
  його та `/dev/kfd`+`/dev/dri` через override `docker-compose.gpu.yml`).
...
10. **Мінімальні залежності хостів**: main — лише Docker; agent — лише Docker
    (+ `amd-smi` на хості для GPU). `hostcmd.sh` працює в контейнері, тому
    `amd-smi` та пристрої GPU (`/dev/kfd`, `/dev/dri`) монтуються в контейнер
    override-файлом `docker-compose.gpu.yml` (тільки для хостів з AMD GPU).
...
- WoL працює, лише якщо main у тій самій L2-мережі (або є маршрутизація
  broadcast).
- `amd-smi` має бути встановлений на хості agent для GPU-телеметрії, і агент
  має запускатися з override `docker-compose.gpu.yml` (монтує `amd-smi` та
  `/dev/kfd`+`/dev/dri` в контейнер). Без цього поле `amd_smi` буде порожнім —
  це не помилка.
